...
const KadElectricModal = ({ modalContent, onClose }) => {
	...
	const [showTypeChangeForm, setShowTypeChangeForm] = useState(false);
	const [retryReference, setRetryReference] = useState("");
	const [typeChangeReference, setTypeChangeReference] = useState("");
	...
	const [typeChangeResponse, setTypeChangeResponse] = useState(null);
	...
	const handleShowRetryForm = () => {
		...
		setShowTypeChangeForm(false);
		setRetryResponse(null);
	};

	// Initialize typeChangeReference when showing the form
	const handleShowTypeChangeForm = () => {
		const firstTransactionRef =
			currentModalContent.transactions?.[0]?.referenceNumber || "";
		setTypeChangeReference(firstTransactionRef);
		setShowTypeChangeForm(true);
		setShowRetryForm(false);
		setTypeChangeResponse(null);
	};
	...
	const handleTypeChangeSubmit = async (e) => {
		e.preventDefault();
		setIsTypeChanging(true);
		try {
			const response = await axiosInstance.post(
				"/KadElectric/RetryWithTypeChange",
				{
					reference: typeChangeReference,
				}
			);

			setTypeChangeResponse({
				isSuccess: response.data.isSuccess,
				message:
					response.data.message || "Type change retry initiated successfully",
				data: response.data,
			});

			// Refresh the modal content after successful type change retry
			if (response.data.isSuccess && currentModalContent?.appId) {
				const appResponse = await axiosInstance.get(
					`/Apps/${currentModalContent.appId}`
				);
			...
			console.error("Error retrying with type change:", error);
			if (error.response) {
				setTypeChangeResponse({
					isSuccess: false,
					message:
						error.response.data.message || "Error retrying with type change",
			...
			>
				...
				>
						...
						<div className='mt-5 px-4'>
							...
							>
								<div className='space-y-2'>
									...
									>
										{typeChangeResponse.isSuccess ? "✅ Success" : "❌ Error"}
									</p>
									<p className='text-xs text-gray-700'>
									...
									{!typeChangeResponse.isSuccess && typeChangeResponse.data && (
										<div className='mt-3'>
											{typeChangeResponse.data.error && (
												<p className='text-xs text-red-600'>
													{typeChangeResponse.data.error}
												</p>
											)}
											{typeChangeResponse.data.errorMessage && (
												<p className='text-xs text-red-600'>
													{typeChangeResponse.data.errorMessage}
												</p>
											)}
											{typeChangeResponse.data.errors && (
												<div className='space-y-2'>
													{Object.entries(typeChangeResponse.data.errors).map(
														([key, value]) => (
															<p key={key} className='text-xs text-red-600'>
																<span className='font-medium'>{key}:</span>{" "}
																{value}
					...
					<div className='my-10 px-4'>
						...
						{showTypeChangeForm && (
							<form
								onSubmit={handleTypeChangeSubmit}
								className='space-y-4 border p-4 rounded-md bg-white'
							>
								<h3 className='text-gray-700 text-sm font-semibold mb-2'>
									{paymentType === "Prepaid"
								...
								<div>
									<label
										className='block text-xs text-gray-700 mb-1'
										htmlFor='typeChangeReference'
									>
										Reference Number
									</label>
									<input
										id='typeChangeReference'
										type='text'
										value={typeChangeReference}
										onChange={(e) => setTypeChangeReference(e.target.value)}
										className='w-full border rounded px-2 py-2 text-xs'
										placeholder='Enter reference number'
										required
									/>
								...
								<div className='flex gap-4'>
									...
									>
										{isTypeChanging ? "Submitting..." : "Submit"}
									</button>
									<button
										type='button'
										onClick={() => {
						...
						<div className='flex flex-col space-y-4'>
							<div className='flex space-x-4 text-xs'>
								...
								{currentModalContent.canRetry && (
									<button
										onClick={handleShowTypeChangeForm}
										className='bg-[#0052CC] text-white px-4 py-2 rounded-sm w-full'
									>
										{paymentType === "Prepaid"
											? "RETRY WITH POSTPAID"

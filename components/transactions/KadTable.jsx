...
const KadTransactionTable = ({ filters }) => {
	...
	// Update the condition to include new boolean filters
		...
		if (shouldFetchAllPages) {
			async function fetchAllPages() {
				try {
					...
					// Add new boolean filters if they are set
					if (filters.canRetry !== "")
						queryParams.push(
							`canRetry=${encodeURIComponent(filters.canRetry)}`
						);
					if (filters.canRefund !== "")
						queryParams.push(
							`canRefund=${encodeURIComponent(filters.canRefund)}`
						);
					if (filters.canSendToken !== "")
						queryParams.push(
							`canSendToken=${encodeURIComponent(filters.canSendToken)}`
						);
					if (filters.canSendError !== "")
						queryParams.push(
					...
					// Client‑side filtering for business, date and referenceNumber
			...
			// New boolean filters in the payload
			if (filters.canRetry !== "")
	...
	// Additional client‑side filtering (including new boolean filters)
	...
	const paginationItems = useMemo(() => {
		const pages = [];
		for (let i = 1; i <= dynamicTotalPages; i++) {
			pages.push(i);
		}
		return pages;
	}, [dynamicTotalPages]);
	...
	const columns = useMemo(
		() => [
			...
			},
			{
				...
				accessor: (row) => {
					if (row.prepaidRequest) {
						return row.prepaidRequest.meterNumber;
			...
			},
			{
				...
				accessor: (row) => {
					if (row.prepaidRequest) {
						return row.prepaidRequest.amount;
					} else if (row.postpaidRequest) {
			...
			},
			{
				...
				accessor: (row) => {
					if (row.prepaidRequest) {
						return row.prepaidRequest.telephoneNumber;
					} else if (row.postpaidRequest) {
			...
			<div className='bg-white rounded-lg shadow-md p-4 overflow-x-auto cursor-pointer'>
				...
				<div className='flex flex-col sm:flex-row justify-between text-xs items-center p-4 bg-gray-50 border-t border-gray-300 space-y-2 sm:space-y-0'>
					...
					<div className='flex items-center space-x-2'>
						...
						<button
							onClick={() => handlePageChange(pageNumber + 1)}
							disabled={pageNumber >= dynamicTotalPages}
							className={`px-1 py-1 border rounded-md ${
								pageNumber >= dynamicTotalPages
									? "bg-gray-200 text-gray-400 cursor-not-allowed"
									: "bg-black text-white hover:bg-gray-100"
							}`}
						>

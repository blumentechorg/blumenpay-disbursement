...
//       try {
//         const token = Cookies.get("tokens.accessToken"); // Get token from cookies
//         const response = await fetch("https://blumenpay-1.onrender.com/Team", {
//           headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // The API returns an object with a `data` key holding the team array.
        const teamData = response.data.data || [];
        // Optionally format the data to add fields needed for the table.
        const formattedData = teamData.map((item) => ({
          ...item,
          // Use the nested user's id for display.
          id: item.user?.user.id || "N/A",
          // Use the nested user's fullName for display.
          fullName: item.user?.user.fullName || "N/A",
          // If your API provides an email, adjust this accordingly.
          email: item.user?.email || "N/A",
          // Use the title as the role.
          role: item.title,
          // Derive status from the active flag.
          status: item.active ? "Active" : "Inactive",
          // If your API provides a last login date, use it; otherwise, set a placeholder.
          // login: item.lastLogin || "N/A",
        }));

...
//   // Fetch data without any external filters
//   useEffect(() => {
//     setIsLoading(true);
...
//   // Only filter based on the search query
//   const filteredData = useMemo(() => {
//     return data.filter((row) => {
//       return searchQuery
//         ? row.referenceNumber.toLowerCase().includes(searchQuery.toLowerCase())
...
//   // Optimized pagination: fixed groups with ellipsis if needed
//   const getPaginationItems = () => {
//     if (dynamicTotalPages <= 8) {
//       return Array.from({ length: dynamicTotalPages }, (_, i) => i + 1);
//     }
...
const ActivityTable = () => {
  ...
  const [selectedRows, setSelectedRows] = useState({});
  const [data, setData] = useState([]);
  // Removed search state from here.
  const [isLoading, setIsLoading] = useState(false);

  // Pagination states (1-indexed)
  const [pageNumber, setPageNumber] = useState(1);
  // Set default pageSize to 2 rows per page.
  ...
  // Use full data as no search filtering is done here.
